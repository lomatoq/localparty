...
function playerView(p){
  return {id:p.id,name:p.name,color:p.color,handedness:p.handedness,x:p.x,y:p.y,vx:p.vx,vy:p.vy,radius:p.radius,alive:p.alive,active:p.active,
    ...
    falseStart:p.falseStart,falseStarts:p.falseStarts,reactionMs:p.reactionMs,bestReactionMs:p.bestReactionMs,reactionCount:p.reactionCount};
}
...
function snapshot(){
  return {world:WORLD,center:CENTER,game:{...game},players:connectedPlayers().map(playerView),
...
function startGame(payload){
  ...
  if(!['push','knives','bomb','western'].includes(mode))return;
...
function nextRound(){
  ...
  if(game.mode==='push')setupPushRound();
...
function setupPushRound(){
  const ps=connectedPlayers();game.arenaRadius=292;game.roundDuration=TEST_FAST?1.2:42;
...
function finishMatch(){
  game.status='finished';game.timer=0;const ps=connectedPlayers();if(!ps.length){game.winnerText='Матч окончен';return;}
  let sorted;
  ...
  else if(game.mode==='western')tied=sorted.filter(p=>p.roundWins===top.roundWins&&Math.abs(averageReaction(p)-averageReaction(top))<1);
...
function endRound(text,winners=[]){
  ...
  for(const p of winners){p.roundWins++;if(game.mode==='push')p.totalScore+=3;}
}

function updatePush(dt){
  const ps=activePlayers().filter(p=>p.alive),elapsed=game.roundDuration-game.timer,shrinkT=clamp((elapsed-(TEST_FAST?.2:12))/(TEST_FAST?.7:24),0,1);game.arenaRadius=292-shrinkT*88;
  for(const p of ps){
    ...
    const accel=jm>.05?1050:0;p.vx+=jx*accel*dt;p.vy+=jy*accel*dt;const damping=Math.pow(jm>.05?.92:.75,dt*60);p.vx*=damping;p.vy*=damping;const sp=len(p.vx,p.vy),maxSp=330;if(sp>maxSp){p.vx=p.vx/sp*maxSp;p.vy=p.vy/sp*maxSp;}p.x+=p.vx*dt;p.y+=p.vy*dt;
  }
  ...
  for(const p of ps){const d=Math.hypot(p.x-CENTER.x,p.y-CENTER.y);if(d>game.arenaRadius+p.radius*.42){p.alive=false;p.input.jx=p.input.jy=0;p.lastFeedback='OUT!';p.lastFeedbackAt=Date.now();}}
...
function westernShoot(p){
  ...
  if(western.phase==='waiting'){
    p.shotThisRound=true;p.falseStart=true;p.falseStarts++;p.alive=false;p.lastFeedback='TOO EARLY!';p.lastFeedbackAt=Date.now();broadcastHosts('westernShot',{playerId:p.id,falseStart:true});return;
  }
  if(western.phase!=='draw'||!p.alive)return;
  p.shotThisRound=true;p.reactionMs=Math.max(0,Date.now()-western.drawAt);p.reactionSum+=p.reactionMs;p.reactionCount++;p.bestReactionMs=p.bestReactionMs==null?p.reactionMs:Math.min(p.bestReactionMs,p.reactionMs);p.lastFeedback=`${p.reactionMs} ms`;p.lastFeedbackAt=Date.now();western.shots.push({playerId:p.id,reactionMs:p.reactionMs});broadcastHosts('westernShot',{playerId:p.id,reactionMs:p.reactionMs});
}
function resolveWestern(){
  ...
  if(!valid.length)return endRound('🤠 Никто не выстрелил после DRAW!',[]);
  const best=valid[0].s.reactionMs,winners=valid.filter(x=>x.s.reactionMs===best).map(x=>x.p);endRound(winners.length>1?`🤠 Ничья · ${best} ms`:`🤠 ${winners[0].name} · ${best} ms`,winners);
}
function updateWestern(dt){
  if(western.phase==='waiting'){
    ...
    western.resolveTimer-=dt;const eligible=activePlayers().filter(p=>p.alive&&!p.falseStart);if(eligible.length&&eligible.every(p=>p.shotThisRound))western.resolveTimer=Math.min(western.resolveTimer,TEST_FAST?.03:.18);if(western.resolveTimer<=0)resolveWestern();
  }
  if(game.timer<=0&&game.status==='playing')resolveWestern();
}
...
function updateGame(dt){
  ...
  if(game.status==='playing'){
    ...
    if(game.mode==='push')updatePush(dt);else if(game.mode==='knives')updateKnives(dt);else if(game.mode==='bomb')updateBomb(dt);else if(game.mode==='western')updateWestern(dt);return;
  }
...
class WSClient{
  constructor(socket){this.socket=socket;this.id=`s${nextSocketId++}`;this.data={};this.buffer=Buffer.alloc(0);this.closed=false;clients.add(this);clientsById.set(this.id,this);socket.on('data',c=>this.onData(c));socket.on('close',()=>this.onClose());socket.on('end',()=>this.onClose());socket.on('error',()=>this.onClose());}
...
function handleMessage(socket,msg){
  ...
  if(event==='join'){
    ...
    if(!p)p=makePlayer(socket,d);if(game.status==='lobby'||game.status==='finished')p.active=false;
  ...
  else if(event==='westernShoot'&&p)westernShoot(p);
...
function handleDisconnect(socket){
  const p=players.get(socket.data.playerId);if(p&&p.socketId===socket.id){p.connected=false;p.input.jx=p.input.jy=0;if(game.mode==='bomb'&&bomb.holderId===p.id){bomb.holderId=null;bomb.phase='cooldown';bomb.resetTimer=TEST_FAST?.04:.35;}broadcast('lobby',lobbyState());}
}
