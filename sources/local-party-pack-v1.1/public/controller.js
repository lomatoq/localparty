...
$('fireBtn').addEventListener('pointerdown',e=>{e.preventDefault();socket.emit('westernShoot');if(navigator.vibrate)navigator.vibrate(18);});
function modeName(mode){return mode==='push'?'PUSH PIT':mode==='knives'?'COLOR KNIVES':mode==='bomb'?'BOMB TAG':mode==='western'?'ONE SHOT WESTERN':'LOBBY';}
...
function applySelf(s){
  self=s;$('modeLabel').textContent=modeName(s.mode);$('roundLabel').textContent=s.mode?`${s.round}/${s.maxRounds}`:'—';flashFeedback(s);
  const inRound=s.mode&&(s.status==='countdown'||s.status==='playing');hideControls();waiting.classList.toggle('hidden',!!inRound);

  if(inRound&&(s.mode==='push'||s.mode==='bomb')){
  ...
  if(inRound&&s.mode==='western'){
    ...
    const locked=s.status!=='playing'||s.falseStart||s.shotThisRound;$('fireBtn').classList.toggle('locked',locked);
