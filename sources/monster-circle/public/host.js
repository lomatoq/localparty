...
function render(s){
  ...
  $('#startGame').disabled=s.players.length!==s.maxPlayers || s.players.length<2;
...
function compose(images,segments){
  const c=$('#revealCanvas'); const w=720, h=520*images.length; c.width=w;c.height=h;
  const ctx=c.getContext('2d'); ctx.fillStyle='#f6efe3';ctx.fillRect(0,0,w,h);
  images.forEach((img,i)=>ctx.drawImage(img,0,i*520,w,520));
  for(let i=1;i<images.length;i++){ctx.fillStyle='#f6efe3';ctx.fillRect(0,i*520-1,w,2)}
}
