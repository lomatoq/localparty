...
function renderLobby(){
  const ps=state.players||[]; $('#playerCount').textContent=ps.length; $('#startBtn').disabled=ps.filter(p=>p.connected).length<3;
...
function renderReveal(){ const ps=state.players||[]; const ready=ps.filter(p=>p.ready).length; $('#readyCount').textContent=`${ready} / ${ps.length}`; $('#readyGrid').innerHTML=ps.map(p=>`<div class="ready-item ${p.ready?'ready':''}"><span>${esc(p.emoji)}</span><b>${esc(p.name)}</b><small>${p.ready?'готов':'смотрит роль…'}</small></div>`).join(''); }
...
function renderResult(){
  ...
  if(r.reason==='spy_failed_guess'){title='Шпион ошибся';icon='💥';text='Шпион рискнул угадать локацию и промахнулся.'}
...
socket.emit('host:hello'); socket.on('state:public',s=>{state=s;localSettings={...s.settings};render()});
