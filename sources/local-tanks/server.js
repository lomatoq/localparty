...
function makePlayer(socket, payload) {
  ...
  const p = {
    ...
    color: playerColors[(nextPlayerNumber - 2) % playerColors.length],
    team: chooseTeam(),
    x: 100,
    y: 100,
    angle: 0,
...
function spawnFor(p, index = 0) {
  ...
  if (game.mode === 'ctf') {
    ...
    const spots = [[95, 275], [95, 355], [95, 435], [155, 355], [155, 275], [155, 435]];
    const s = spots[index % spots.length];
    p.x = s[0]; p.y = s[1]; p.angle = 0;
  } else {
    const spots = [[110,110],[1170,110],[110,610],[1170,610],[365,105],[915,615],[365,615],[915,105]];
    const s = spots[index % spots.length];
    p.x = s[0]; p.y = s[1];
...
function snapshot() {
  return {
    ...
    bullets: bullets.map(b => ({ id:b.id,x:b.x,y:b.y,radius:b.radius,color:b.color })),
