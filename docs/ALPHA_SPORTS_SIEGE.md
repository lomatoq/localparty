...
The public `lomatoq/localparty` repository updates anonymously and needs no key or GitHub sign-in. For a private fork, or after GitHub's anonymous API limit is exhausted, the host can optionally use `gh auth login` or `PARTY_GITHUB_TOKEN` with **Contents: read**. Credentials are attempted only after anonymous access fails and are never sent to phones or redirected download hosts.

Only a localhost host session with the host key can install. Installation requires confirmation and an idle lobby. A game cannot start during installation. A git installation must have no tracked changes; unrelated untracked files are preserved, and Git still refuses a checkout if one would be overwritten. The updater fetches and checks out the pinned revision **detached**, without force-moving the experimental or stable branch. If dependency locks differ, the safe updater refuses and asks for a separate `npm ci` install instead of mutating a live dependency tree.
