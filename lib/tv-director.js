...
const {rankResultRows}=require('./result-ranking');
const LIMIT = 16;
...
function matchRows(result, knownPlayers) {
  ...
  const rows=(Array.isArray(result?.players)?result.players:[]).slice(0,64).flatMap(r=>{
  ...
  return rankResultRows(rows).slice(0,LIMIT);
}
...
class TVDirector {
  constructor({catalog, store}) {
    ...
    this.options={autoPodium:saved.autoPodium===true,effects:saved.effects!==false,idleBrowse:saved.idleBrowseSet===true&&saved.idleBrowse===true};
    // Idle catalog scrolling is opt-in: older saves recorded idleBrowse:true only as a default.
  ...
  command(m, {active=null,busy=false,selected=null,sharing=false}={}) {
    ...
    if(m.type==='tv-options') {
      ...
      const chosen=Object.hasOwn(patch,'idleBrowse')||this.store.data.tvOptions?.idleBrowseSet===true;
      Object.assign(this.options,patch);this.store.data.tvOptions={...this.options,...(chosen?{idleBrowseSet:true}:{})};this.store.save();
      ...
      if(Object.hasOwn(m,'id')) {
        // The remote walks cards in the order the TV shows them (Fresh shelf, featured, …).
        index=typeof m.id==='string'?this.catalog.findIndex(g=>g.id===m.id):-1;
        if(index<0)throw Error('Такой игры нет в каталоге.');
      } else if(Object.hasOwn(m,'number')) {
