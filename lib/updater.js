...
function download(url,auth,{max=256*1024*1024,accept='application/vnd.github+json',redirect=0,onProgress}={}){
 ...
 return new Promise((resolve,reject)=>{
  ...
  const req=https.get(u,{headers,timeout:30000},res=>{
   ...
   if(res.statusCode!==200){res.resume();const error=Error([401,403,404].includes(res.statusCode)?'GitHub не отдал запрошенную версию. Проверьте интернет, ветку и лимит GitHub API.':'GitHub: HTTP '+res.statusCode);error.statusCode=res.statusCode;reject(error);return;}
...
async function withPublicFallback(action,getToken=token){
 try{return await action('');}catch(error){
  if(![401,403,404].includes(error.statusCode))throw error;
  const auth=await getToken();if(!auth)throw error;
  return action(auth);
 }
}
const githubDownload=(url,options)=>withPublicFallback(auth=>download(url,auth,options));
function createUpdater({root,hostKey,isLocal,isBusy,getPort,shutdown}){
 ...
 async function sourceInfo(){if(!fs.existsSync(path.join(root,'.git')))return null;return {previous:await git(['rev-parse','HEAD']),dirty:!!(await git(['status','--porcelain','--untracked-files=no']))};}
 async function fetchRevision(revision){
  const args=['-C',root,'-c','credential.helper=','fetch','--no-tags','https://github.com/'+REPO+'.git',revision],env={...process.env,GIT_TERMINAL_PROMPT:'0'};
  try{await exec('git',args,{env,timeout:120000,windowsHide:true,maxBuffer:4*1024*1024});return;}catch(publicError){
   const auth=await token();if(!auth)throw publicError;
   env.GIT_CONFIG_COUNT='1';env.GIT_CONFIG_KEY_0='http.https://github.com/.extraheader';env.GIT_CONFIG_VALUE_0='AUTHORIZATION: basic '+Buffer.from('x-access-token:'+auth).toString('base64');
   await exec('git',args,{env,timeout:120000,windowsHide:true,maxBuffer:4*1024*1024});
  }
 }
 async function check(channel){
  if(!['stable','alpha'].includes(channel))throw Error('Неизвестный канал');
  const info=await sourceInfo();
  if(info?.dirty)throw Error('Есть изменения в файлах проекта. Сохраните их перед обновлением; они не будут перезаписаны.');
  const remote=JSON.parse((await githubDownload(channel==='alpha'?API+'/commits/'+encodeURIComponent(BRANCH):API+'/releases/latest',{max:3*1024*1024})).toString());
  ...
  if(channel==='stable'){
   const commit=JSON.parse((await githubDownload(API+'/commits/'+encodeURIComponent(remote.tag_name),{max:3*1024*1024})).toString());
 ...
 async function prepare(id,rollback=false){
  ...
  try{
   ...
   const c=candidate;Object.assign(plan,{revision:c.revision,channel:c.channel});
    progress('downloading','Загружаем '+c.version+'…');
    if(info){
     // Public repositories are fetched anonymously. Credentials remain an optional fallback for private forks.
     await fetchRevision(c.revision);
     ...
     if(c.channel==='stable'){
      ...
      if(!hash){const side=c.assets.find(a=>a.name===asset.name+'.sha256');if(!side)throw Error('У релиза нет SHA-256');hash=(await githubDownload(side.url,{max:1024,accept:'application/octet-stream'})).toString().trim().split(/\s+/)[0];}
      if(!/^[a-f0-9]{64}$/i.test(hash))throw Error('Неверная контрольная сумма релиза');full=true;
     }
     const bytes=await githubDownload(address,{accept:'application/octet-stream',onProgress:(n,total)=>progress('downloading','Загружаем '+c.version+'…',{bytes:n,total})});
...
module.exports={createUpdater,dependencyIdentity,allowedDownload,cacheFor,withPublicFallback};
