...
function schema(game){const engine=game.engine||game.id;const settings=[...(fields[engine]||[])];if(game.id==='shrink')settings.push(choice('shrinkSpeed','Сужение арены',[['normal','Обычное'],['fast','Быстрое']],'normal'));return {settings,actions:(actions[engine]||[]).map(([id,label,phases])=>({id,label,phases}))};}
