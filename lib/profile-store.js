...
class ProfileStore{
 ...
 register(token,name,hand){let p=this.get(token);if(!p){p={id:crypto.randomBytes(8).toString('hex'),token:crypto.randomBytes(24).toString('hex'),createdAt:Date.now(),stats:{played:0,wins:0,points:0,games:{}}};this.data.players.push(p);}const changed=p.name!==name||p.hand!==hand||!p.lastSeen;p.name=name;p.hand=hand;p.lastSeen=Date.now();if(changed)this.save();return p;}
