...
class ProfileStore{
 ...
 record(result,session,game,revision=this.statisticsRevision||0){
  ...
  const ranked=rankResultRows(valid);this.data.completed=(this.data.completed||this.data.events.length)+1;this.data.events.push({key:eventKey,game,at:Date.now(),duration:result.duration||0,players:ranked});if(this.data.events.length>500)this.data.events.splice(0,this.data.events.length-500);this.save();return true;
 }
