...
class NetworkAccess {
 constructor(handler,upgrade,{port=8080,provision=null,address=()=>null}={}) {this.handler=handler;this.upgrade=upgrade;this.preferredPort=port;this.provision=provision;this.address=address;this.hostname='';this.boundAddress='';this.port=0;this.server=null;this.sockets=new Set();this.tail=Promise.resolve();}
 ...
 async open(){
  if(this.server&&this.boundAddress===this.address())return;
  if(this.server)await this.close();
  const ip=this.address();
  const certificate=this.provision?await this.provision(ip):null;
  const server=certificate?https.createServer({key:certificate.key,cert:certificate.cert},(req,res)=>{req.partyPublic=true;this.handler(req,res);}):http.createServer((req,res)=>{req.partyPublic=true;this.handler(req,res);});
  ...
  this.server=server;this.port=server.address().port;this.preferredPort=this.port;this.hostname=certificate?.hostname||'';this.boundAddress=ip||'';
 }
 async close(){
  const server=this.server;if(!server)return;
  this.server=null;this.port=0;this.hostname='';this.boundAddress='';
