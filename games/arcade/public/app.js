'use strict';
const $=id=>document.getElementById(id),host=!!window.IS_HOST;let ws,state,id,joinRetry=0,axis={x:0,y:0},motionEnabled=false,motionPeak=0,lastMotion=0,punchReady=false,punchArmed=false,punchTurnSince=0,punchStillSince=0,punchCue='';
...
function renderArcadeBoard(s){
 ...
 ordered.forEach((p,i)=>{
  ...
  const {avatar,label,detail,score}=row.parts;text(avatar,['😎','🦊','🐸','🐻'][s.players.indexOf(p)%4]);
...
function renderPunchPhone(s,p){
 ...
 $('status').textContent=s.phase==='finished'?'Твои результаты':s.phase!=='playing'?'Готовимся к ударам':mine?'Твой удар':'Бьёт: '+(turn?.name||'—');
 ...
 $('motion').onclick=async()=>{
  ...
  try{
   ...
   motionListener=e=>{
    ...
    // Lower scoring gain, not the safe arming threshold: no need to swing harder.
    if(magnitude<3&&motionPeak>sense.trigger&&now-lastMotion>1800){send('input',{action:'punch',side:Math.max(-1,Math.min(1,motionSide)),power:Math.max(0,Math.min(1,(motionPeak-sense.trigger*.5)/(sense.full-sense.trigger*.5)))/5});motionPeak=0;lastMotion=now;punchReady=false;punchArmed=false;punchStillSince=0;}
