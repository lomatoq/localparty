...
const controllerView=require('./controller-view');

runtime.onPause(()=>game.players.forEach(p=>p.input={x:0,y:0}));
runtime.setInterval(()=>{game.tick(1/30);if(game.phase==='finished'&&!reported){reported=true;const best=Math.max(...game.players.map(p=>p.score));runtime.report({eventId,gameId:game.mode,duration:(Date.now()-started)/1000,players:game.players.map(p=>({id:p.id,name:p.name,score:p.score,won:p.score===best,metrics:require('./report-metrics')(game,p)}))});}runtime.ui({phase:game.phase==='lobby'?'waiting':game.phase==='finished'?'results':'playing',label:'До финиша',endsAt:game.phase==='playing'?Date.now()+game.timer*1000:null,progress:game.mode==='snakelines'?`Раунд ${game.round}/5`:''});const v=state(),controllerState=controllerView(v),controllerKey=JSON.stringify(controllerState),hostPacket=game.mode==='snakelines'?null:JSON.stringify({type:'state',data:v});for(const ws of wss.clients){if(!ws.host&&!ws.pid)continue;if(!ws.host){if(ws.readyState===1&&ws.bufferedAmount<256*1024&&ws.controllerKey!==controllerKey){ws.send(JSON.stringify({type:'state',data:{...controllerState,selfId:ws.pid}}));ws.controllerKey=controllerKey;}continue;}if(game.mode==='snakelines'){if(ws.readyState!==1||ws.bufferedAmount>=256*1024)continue;ws.trailOffsets??={};const reset=ws.trailMatch!==eventId+':'+game.round;ws.trailMatch=eventId+':'+game.round;const players=v.players.map(p=>{const from=reset?0:Math.min(ws.trailOffsets[p.id]||0,p.trail.length);ws.trailOffsets[p.id]=p.trail.length;return {...p,trailFrom:from,trail:ws.host?p.trail.slice(from):[]};});send(ws,'state',{...v,players});}else if(ws.readyState===1)ws.send(hostPacket);}},1000/30);
...
runtime.host({start:()=>{if(!game.start())return false;if(process.env.TEST_FAST==='1')game.timer=2;eventId=crypto.randomUUID();started=Date.now();reported=false;return true;}});
