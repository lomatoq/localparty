...
function bindHold(el,key){
  const down=e=>{e.preventDefault();if(el.disabled)return;input[key]=true;el.classList.add('pressed');socket.emit('input',input);if(key==='fire'&&navigator.vibrate)navigator.vibrate(12);try{el.setPointerCapture(e.pointerId)}catch{}};
