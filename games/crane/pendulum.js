...
class HangingLoad {
 ...
 reset(angle=.1){this.angle=angle;this.velocity=0;}
 step(dt,anchorAcceleration=0){const steps=Math.max(1,Math.ceil(dt/(1/120))),h=dt/steps;for(let i=0;i<steps;i++){const acceleration=-(13.2*80/this.length)*Math.sin(this.angle)-(anchorAcceleration/this.length)*Math.cos(this.angle)-.45*this.velocity;this.velocity+=acceleration*h;this.angle+=this.velocity*h;const limit=.52;if(Math.abs(this.angle)>limit){this.angle=Math.sign(this.angle)*limit;this.velocity*=-.18;}}}
