...
let matchId='',matchStarted=0,reported=false;
const app = express();
...
function publicPlayer(p) {
  return { id: p.id, name: p.name, emoji: p.emoji, connected: p.connected, ready: !!p.ready, spectator:!!p.spectator, hasVoted: state.voteBy.has(p.id) };
}
...
function emitPublic() {
  runtime.ui?.({phase:state.phase==='lobby'?'waiting':state.phase==='result'?'results':state.phase==='reveal'||state.phase==='voting'?'playing':state.phase,endsAt:state.phase==='playing'?state.endsAt||null:null,label:state.phase==='reveal'?'Раздача ролей':state.phase==='voting'?'Голосование':'До обсуждения',progress:state.phase==='reveal'?'Посмотрите роль на телефоне':state.duel?'Мини-режим: вопрос и догадка':'Найдите шпиона'});
  if(state.phase==='result'&&!reported){reported=true;runtime.report({gameId:'spy',eventId:matchId,duration:(Date.now()-matchStarted)/1000,players:[...state.players.values()].filter(p=>p.participant).map(p=>({id:p.partyId||p.id,name:p.name,score:(state.spies.has(p.id)?state.result.winner==='spies':state.result.winner==='citizens')?1:0,won:state.spies.has(p.id)?state.result.winner==='spies':state.result.winner==='citizens',metrics:{spyRounds:state.spies.has(p.id)?1:0,votes:state.voteBy.has(p.id)?1:0}}))});}
...
function emitPrivate(player) {
  ...
  if (state.location && state.phase !== 'lobby' && !player.spectator) {
    if (isSpy) {
      privateState.secret = {
        title: 'ТЫ ШПИОН 🕵️',
...
function alivePlayers() { return [...state.players.values()].filter(p => p.connected && !p.spectator); }
...
function startPlaying() {
  ...
  clearRoundTimer();
  state.timer = runtime.setTimeout(() => beginVoting('time'), state.settings.minutes * 60_000);
  emitPublic(); emitAllPrivate();
}
function beginVoting(source = 'host') {
  if (!['playing','reveal'].includes(state.phase)) return;
  if(state.duel&&source!=='time')return;
  if(state.duel){state.result={winner:'citizens',reason:'duel_timeout',accusedIds:[]};state.phase='result';state.endsAt=null;clearRoundTimer();emitPublic();emitAllPrivate();return;}
  state.phase = 'voting';
  state.endsAt = null;
  clearRoundTimer();
  state.voteBy.clear();
...
function resetToLobby() {
  ...
  state.turnIndex = 0;
  state.endsAt = null;
  for (const p of state.players.values()) { p.ready = false; p.role = null;p.spectator=false;p.participant=false; }
  emitPublic(); emitAllPrivate();
}
function startRound() {
  const players = [...state.players.values()].filter(p=>p.connected);
  if (players.length < 2) return { ok:false, error:'Нужно минимум 2 игрока' };
  clearRoundTimer();state.duel=players.length===2;matchId=crypto.randomUUID();matchStarted=Date.now();reported=false;for(const p of state.players.values()){p.spectator=!p.connected;p.participant=p.connected;}
...
io.on('connection', (socket) => {
  const trustedHost=runtime.managed?socket.handshake?.headers?.['x-party-local']==='1':['127.0.0.1','::1','::ffff:127.0.0.1'].includes(socket.handshake?.address);
  socket.use?.(([event],next)=>{if(String(event).startsWith('host:')&&!trustedHost)return;next();});
  ...
  socket.on('player:join', (payload = {}, cb = () => {}) => {
    ...
    if (!player) {
      player = { id:playerId, name:cleanName, emoji:String(emoji||'🙂').slice(0,4), socketId:socket.id, connected:true, ready:false, role:null,spectator:state.phase!=='lobby',participant:false };
    ...
    if(identity)player.partyId=identity.id;
    socket.join('players');
  ...
  socket.on('player:ready', () => {
    ...
    if (!p || p.socketId!==socket.id || p.spectator || state.phase !== 'reveal') return;
    p.ready = true;
    emitPublic(); emitPrivate(p);
    const active = alivePlayers();
    if (active.length >= 2 && active.every(x => x.ready)) startPlaying();
  });

  socket.on('player:vote', ({ targetId } = {}, cb=()=>{}) => {
    const voterId = socket.data.playerId;const voter=state.players.get(voterId);if(!voter||voter.spectator||voter.socketId!==socket.id||state.voteBy.has(voterId)||state.players.get(targetId)?.spectator)return cb({ok:false,error:'Голос недоступен'});
  ...
  socket.on('player:spyGuess', ({ location } = {}, cb=()=>{}) => {
    const playerId = socket.data.playerId;if(state.players.get(playerId)?.socketId!==socket.id)return;
  ...
  socket.on('player:nextTurn', () => {
    ...
    if (turn?.askerId === id && state.players.get(id)?.socketId===socket.id) nextTurn();
  });
  ...
  socket.on('disconnect', () => {
    ...
    if (id && state.players.has(id)) {
      ...
      if(p.socketId!==socket.id)return;p.connected = false;if(p.partyId)runtime.presence(p.partyId,false);
      p.socketId = null;
      emitPublic();
    }
  });
...
server.listen(PORT, (process.env.PARTY_MANAGED === '1' ? '127.0.0.1' : '0.0.0.0'), () => {
  PORT=server.address().port;process.send?.({type:'ready',port:PORT});const ip = findLanIp();
  console.log('\n🕵️  SPY LAN GAME');
