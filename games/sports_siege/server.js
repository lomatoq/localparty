...
wss.on('connection',(ws,req)=>{
  ...
  ws.on('message',(raw,binary)=>{
    ...
    if(m.type==='join'){
      let profile=runtime.identify(d,ws),session;
...
runtime.host({start:settings=>{if(match.phase!=='waiting')return false;const started=match.start(settings);broadcast();return started;}});
