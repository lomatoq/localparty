...
export class Renderer {
 ...
 drawTanks(c,s,dt){const bullets=s.projectiles||[],target=bullets.length?bullets[0]:null;const minY=target?Math.min(0,target.y-50):0;const maxY=Math.max(720,...s.players.filter(p=>p.participant).map(p=>p.y+90),target?target.y+80:720),z=clamp(700/(maxY-20-minY),.3,1),y=(minY+maxY)/2;const ease=1-Math.exp(-dt*5);this.cam.z=lerp(this.cam.z,z,this.reduced?1:ease);this.cam.y=lerp(this.cam.y,y,this.reduced?1:ease);this.cam.x=640;c.translate(640,360);c.scale(this.cam.z,this.cam.z);c.translate(-this.cam.x,-this.cam.y);
