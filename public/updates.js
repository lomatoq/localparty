(() => {
 ...
 dialog.innerHTML=`<form method="dialog"><button class="quiet lp-update-close" aria-label="Закрыть">×</button></form><div class="eyebrow">НОВОЕ ДЛЯ ВАШЕЙ КОМПАНИИ</div><h2 id="lp-updates-title">Ещё один <em>апдейт.</em></h2><p id="lp-update-current"></p><fieldset class="lp-update-channels"><legend>Откуда обновляться</legend><label><input type="radio" name="lp-channel" value="stable"><b>Релиз</b><small>Последняя стабильная версия GitHub</small></label><label><input type="radio" name="lp-channel" value="alpha" checked><b>Экспериментальная <i>alpha</i></b><small>Новые игры из alpha/party-sports-siege</small></label></fieldset><p id="lp-update-warning">Alpha может содержать ошибки. Перед заменой создаётся резервная копия; профили остаются на компьютере.</p><div class="lp-update-status" id="lp-update-status" role="status" aria-live="polite">Проверка запускается только по кнопке. Для игры интернет не нужен.</div><progress id="lp-update-progress" hidden></progress><label class="lp-update-confirm"><input id="lp-update-confirm" type="checkbox">Разрешаю перезапуск сервера после установки</label><div class="lp-update-actions"><button class="quiet" id="lp-update-check">Проверить</button><button class="primary" id="lp-update-install" disabled>Скачать и обновить ↓</button></div><button class="quiet" id="lp-update-rollback" hidden>↶ Вернуть предыдущую установку</button><details><summary>Приватный GitHub и резервные копии</summary><p>Для lomatoq/localparty на компьютере нужен вход через <code>gh auth login</code> или переменная <code>PARTY_GITHUB_TOKEN</code> с доступом Contents: read. Токен не отправляется на телефоны. Резервные копии и журнал находятся в <code>~/.localparty-updates</code>.</p><p>Обновление git-копии возможно только без локальных изменений и без смены зависимостей. Старый стабильный релиз может не содержать эту кнопку и четыре alpha-игры.</p></details>`;
