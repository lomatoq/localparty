(()=>{'use strict';
const $=id=>document.getElementById(id);let ws,state,key='',reconnect,offset=0,selected='',lastPlayers='',accessClosed=false;
...
const sharing=state.networkEnabled!==false&&state.urls.length>0;for(const id of ['qr','qrCaption','address'])$(id).hidden=!sharing;text('inviteHint',sharing?'Одна сеть Wi-Fi. Каждый телефон — контроллер.':'Для телефонов друзей включите «Доступ по Wi-Fi» в комнате на iPhone.');
const join=state.urls[0]||'';if($('address').textContent!==join){$('address').textContent=join;$('qr').src='/api/qr?url='+encodeURIComponent(join);}
...
function connect(){clearTimeout(reconnect);const channel=ws=new WebSocket((location.protocol==='https:'?'wss:':'ws:')+'//'+location.host+'/lobby');channel.onopen=()=>{if(ws!==channel)return;channel.send(JSON.stringify({type:'display',key:window.PARTY_DISPLAY_KEY}));$('connection').textContent='Подключаем экран…';};channel.onmessage=e=>{if(ws!==channel)return;const m=JSON.parse(e.data);if(m.type==='display-ok')$('connection').textContent='Экран подключён к iPhone';if(m.type==='error'&&m.code==='DISPLAY_AUTH'){location.reload();return;}if(m.type==='access-closed'){accessClosed=true;state={...state,active:null,networkEnabled:false,urls:[]};render();$('connection').textContent='Ждём приглашения ведущего';}
if(m.type==='state'){accessClosed=false;state=m;if(m.active?.ui?.serverNow)offset=Date.now()-m.active.ui.serverNow;render();}if(m.type==='game-ui'&&m.instance===state?.active?.instance){state.active.ui=m.ui;offset=Date.now()-m.ui.serverNow;hud();}if(m.type==='session-start')hud();if(m.type==='error')tell(m.message);};channel.onclose=()=>{if(ws!==channel)return;$('connection').textContent=accessClosed?'Доступ по Wi-Fi закрыт ведущим':'Подключаем экран заново…';reconnect=setTimeout(connect,1200);};channel.onerror=()=>{};}
function fitScreen(){
