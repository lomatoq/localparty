...
const signature=JSON.stringify(state.players.map(p=>[p.id,p.name]));if(signature!==lastPlayers){lastPlayers=signature;$('count').textContent=state.players.length?state.players.length+' в комнате':'Ждём компанию';$('players').replaceChildren(...state.players.map(p=>{const n=document.createElement('span');n.className='person';n.textContent=p.name;return n;}));}
}
...
function fitScreen(){var play=$('play'),bar=play.querySelector('.gamebar');var top=play.getBoundingClientRect().top;var height=Math.max(1,window.innerHeight-top);play.style.height=height+'px';$('gameFrame').style.height=Math.max(1,height-bar.offsetHeight)+'px';}
