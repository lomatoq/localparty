...
(()=>{
 ...
 let current=location.protocol==='partyapp:'?'games':'controller',savedScroll=0,dock;
 ...
 function select(tab,notify=true){
  ...
  if(host&&tab!=='controller'){
   ...
   if(tab==='host'&&current!=='host')savedScroll=scrollY;
   document.body.classList.toggle('native-host-tab',tab==='host');
   if(tab==='host'){if(host.open)host.close();host.show();window.scrollTo(0,0);}else{if(host.open)host.close();window.scrollTo(0,savedScroll);}
 ...
 function mount(){
  ...
  const style=document.createElement('style');style.textContent=`
   ...
   html body.native-host-tab #hostPanel[open]{z-index:30}
   ...
   body.native-host-tab>main,body.native-host-tab>.native-run{display:none!important}
   html body.native-host-tab #hostPanel[open]{position:fixed;inset:var(--host-head,76px) 0 var(--native-tab-reserve);width:100%;max-width:none;height:auto;max-height:none;margin:0;padding:20px 16px 36px;border:0;border-radius:0;background:#0d1017;box-shadow:none;overflow:auto;animation:none!important}
  ...
  `;if(window.__partyPersistentTabs){style.textContent+=' #partyNativeDock.native-tabs{visibility:hidden!important;pointer-events:none!important}';dock.setAttribute('aria-hidden','true');}document.head.append(style);paint();window.dispatchEvent(new Event('resize'));
 }
