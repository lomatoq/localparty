(() => {
 ...
 function connect(){
  ...
  ws.onclose=()=>{if(ws!==channel)return;accepted=false;$('joinForm').querySelector('button[type="submit"]').disabled=!replaced;$('connection').textContent=replaced?'Другая вкладка':'Восстанавливаем связь…';$('connection').classList.remove('online');if(!replaced)reconnectTimer=setTimeout(connect,800);};
  ws.onerror=()=>{};
 }
 ...
 $('joinForm').onsubmit=e=>{e.preventDefault();const data={type:'join',token:profile?.token,freshIdentity:freshIdentityPending,name:$('name').value,hand:document.querySelector('[name=hand]:checked').value,avatar:pendingAvatar||null};if(replaced){replaced=false;connect();ws.addEventListener('open',()=>send(data),{once:true});}else send(data);};
