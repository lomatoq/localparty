...
const device=process.env.PARTY_TEST_DEVICE||'B99A3347-E6C7-4D7B-8FBC-DF3F38DA90C4',base='http://127.0.0.1:8080',key='localparty-integration-test',all=[];
...
(async()=>{try{
 ...
 await manage({type:'server-start'});await manage({type:'stop'});for(const p of (await manage()).players)await manage({type:'kick',id:p.id});
 ...
 const after=await manage();assert.equal(after.enabled,true);assert.equal(after.active,null);assert.ok(after.incident.message.includes('прошлый матч'));assert.equal(after.selected,'tanks');assert.equal(after.gameSettings.tanks.mode,'ctf');
 for(const g of guests){const resume=await socket(g.profile);assert.equal(resume.profile.id,g.profile.id);}
 await until(async()=>(await manage()).votes.length===4,'restored votes');assert.equal((await manage()).players.length,4);
 console.log('PASS actual iPhone app restart: automatic room recovery, interrupted-match notice, four identities, votes and CTF settings');
 await manage({type:'dismiss-incident'});await manage({type:'server-stop'});launch();await until(async()=>await manage(),'clean restart');assert.equal((await manage()).enabled,false);assert.equal((await manage()).incident,null);
 console.log('PASS explicit Stop stays stopped after app restart');
