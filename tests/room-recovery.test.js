...
test('16 guests: votes, duplicate sockets, removal, process loss, quiet recovery and automatic local readiness',{timeout:45000},async()=>{
 const dir=fs.mkdtempSync(path.join(os.tmpdir(),'party-recovery-'));let child,origin,screen;const sockets=[];
 async function boot(){let log='';child=spawn(process.execPath,['server.js'],{cwd:path.join(__dirname,'..'),env:{...process.env,PARTY_EMBEDDED:'1',PARTY_ADMIN_KEY:'test',PARTY_DATA_FILE:path.join(dir,'party.json'),PARTY_PORT:'0',PARTY_INTERNAL_PORT:'0',PARTY_NO_BROWSER:'1'},stdio:['ignore','pipe','pipe']});child.stdout.on('data',d=>log+=d);child.stderr.on('data',d=>log+=d);await until(()=>/localhost:(\d+)/.test(log),'server startup '+log);origin='http://127.0.0.1:'+log.match(/localhost:(\d+)/)[1];}
 ...
 try{
  await boot();assert.equal((await command()).enabled,true);assert.equal((await command()).networkEnabled,false);
  ...
  await kill();await boot();let recovered=await command();assert.equal(recovered.enabled,true);assert.equal(recovered.incident,null);assert.equal(recovered.networkEnabled,false);assert.notEqual(recovered.bootId,oldBoot);assert.equal(recovered.selected,'tanks');assert.equal(recovered.gameSettings.tanks.mode,'ctf');
  for(let i=1;i<16;i++){const resumed=await connect(guests[i].profile);assert.equal(resumed.profile.id,guests[i].profile.id);}
  assert.equal((await command()).players.length,15);assert.equal((await command()).votes.length,0);assert.ok(!(await connect(original)).profile);
  await command({type:'dismiss-incident'});assert.equal((await command()).incident,null);
  await command({type:'server-stop'});await kill();await boot();recovered=await command();assert.equal(recovered.enabled,true);assert.equal(recovered.networkEnabled,false);assert.equal(recovered.incident,null);
