...
(async()=>{
  const browser=await chromium.launch({headless:true,executablePath:process.env.CHROMIUM_PATH||undefined,args:['--no-sandbox','--enable-unsafe-swiftshader','--use-angle=swiftshader','--disable-accelerated-2d-canvas']});const report={browser:await browser.version(),cases:[],errors:[]};
  try{
    for(const mode of ['curling','bowling','gate_siege','pop_shots']){
      const run=await launchGame(mode),contexts=[];let host,phone;try{
        const hostContext=await browser.newContext({viewport:{width:1600,height:1000}});contexts.push(hostContext);host=await hostContext.newPage();host.on('pageerror',e=>report.errors.push(mode+':host:'+e.message));
        ...
        await host.bringToFront();await host.waitForFunction(()=>!document.querySelector('#start').disabled);await host.locator('#start').click();await host.waitForFunction(()=>document.querySelector('#start').hidden,null,{timeout:20000});
        phone=phones[0];await phone.bringToFront();await phone.waitForFunction(()=>document.querySelector('#ap-you').textContent.includes('Тестер'),null,{timeout:10000,polling:100});
        ...
        if(['curling','bowling'].includes(mode)){
          await phone.waitForFunction(()=>document.querySelector('#ap-state').textContent==='Твой бросок',null,{timeout:10000,polling:100});await phone.locator('#ap-precise-open').click();await phone.locator('#ap-throw').click();await sleep(900);assert.match(await phone.locator('#ap-state').textContent(),/Смотри|Бросает/);
        }else{
        ...
        await phone.screenshot({path:path.join(OUT,mode+'-phone.png')});await host.bringToFront();await host.screenshot({path:path.join(OUT,mode+'-host.png')});await phone.bringToFront();
      ...
      }catch(e){console.error('BROWSER-CASE-FAILED',mode,e.message);throw e;}finally{for(const ctx of contexts)await ctx.close();run.child.kill();fs.writeFileSync(path.join(OUT,mode+'-server.log'),run.log.join(''));}
    }
    const run=await launchGame(null),contexts=[];try{
      ...
      await host.bringToFront();await host.locator('.game[data-id="curling"] .start-game').click();for(const p of phones){await p.bringToFront();await p.locator('#readyButton').waitFor({state:'visible',timeout:20000});await p.locator('#readyButton').click();}
      await host.bringToFront();const f=host.frameLocator('#gameFrame');await f.locator('#ap-status').filter({hasText:/Энд/i}).waitFor({timeout:20000});await host.screenshot({path:path.join(OUT,'managed-curling.png')});
