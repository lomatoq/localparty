...
(async()=>{
  ...
  try{
    for(const mode of ['curling','bowling','gate_siege','pop_shots']){
      const run=await launchGame(mode),contexts=[];let host,phone;try{
        ...
        if(['curling','bowling'].includes(mode)){
          await phone.waitForFunction(()=>document.querySelector('#ap-state').textContent==='Твой бросок',null,{timeout:10000,polling:100});await phone.locator('details summary').click();await phone.locator('#ap-throw').click();await sleep(900);assert.match(await phone.locator('#ap-state').textContent(),/Смотри|Бросает/);
        }else{
    ...
    const run=await launchGame(null),contexts=[];try{
      ...
      await host.bringToFront();const f=host.frameLocator('#gameFrame');await f.locator('#ap-status').filter({hasText:/ЭНД/}).waitFor({timeout:20000});await host.screenshot({path:path.join(OUT,'managed-curling.png')});
