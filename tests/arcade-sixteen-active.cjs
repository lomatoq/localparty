...
for(const id of ['taprace','punchmeter','flappy','hungry','snakelines','carryball']){
 ...
 await host.evaluate(async id=>{window.extraGameSockets?.forEach(s=>s.close());window.extraGameSockets=[];await Promise.all(extraProfiles.map(p=>new Promise(resolve=>{const socket=new WebSocket('ws://'+location.host+'/games/'+id+'/ws');extraGameSockets.push(socket);socket.onopen=()=>socket.send(JSON.stringify({type:'join',data:{partyId:p.id,partyToken:p.token,name:p.name}}));socket.onmessage=e=>{if(JSON.parse(e.data).type==='joined')resolve();};})));},id);await phone.waitForFunction(()=>!document.querySelector('#readyButton').disabled);
 ...
 for(const size of [{width:1920,height:1080},{width:3430,height:1300}]){
  ...
  if(size.width===1920)await host.screenshot({path:'tests/arcade-16-host-'+id+'.png'});
 }
 ...
 await host.setViewportSize({width:1920,height:1080});await frame.locator('#start').click();await host.waitForFunction(()=>qaState.active.ui.phase==='playing');await host.waitForTimeout(250);await host.screenshot({path:'tests/arcade-16-active-'+id+'.png'});const center=await frame.locator('#arena').evaluate(e=>{const r=e.getBoundingClientRect();return r.left+r.width/2-innerWidth/2;});assert(Math.abs(center)<1);await host.evaluate(()=>qa.send(JSON.stringify({type:'stop'})));await host.waitForFunction(()=>!qaState.active);
}
