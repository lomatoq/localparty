...
for(const id of ['kart','warsaw','western_duel']){
 ...
 for(const size of [{width:1920,height:1080},{width:3430,height:1300}]){
  ...
  if(size.width===1920){console.log('ROOT',await host.evaluate(()=>({scroll:[scrollX,scrollY],play:[document.querySelector('#play').getBoundingClientRect().x,document.querySelector('#play').scrollLeft,document.querySelector('#play').scrollTop],frame:document.querySelector('#gameFrame').getBoundingClientRect().toJSON()})));console.log('INNER',await frame.evaluate(()=>({game:document.documentElement.dataset.partyGame,scroll:[scrollX,scrollY],main:getComputedStyle(document.querySelector('main')).padding,root:document.documentElement.className})));}if(size.width===1920)await host.screenshot({path:'tests/layout-host-'+id+'.png'});
 }
