...
async function main(){
  ...
  try{
    ...
    const errors=[],assets=new Set(),host=await browser.newPage({viewport:{width:1440,height:1000}});host.on('pageerror',error=>errors.push(String(error)));host.on('response',response=>{if(response.url().includes('/assets/gameplay/sports-siege/'))assets.add(new URL(response.url()).pathname);});await host.goto(`${base}/host`);
    ...
    const required=['/swarm-ground-tile.webp','/sprites/swarm-termite.webp','/sprites/turret-head-long.webp','/sprites/turret-pedestal-stone.webp','/sprites/wall-straight.webp','/sprites/cover-wood.webp'];
